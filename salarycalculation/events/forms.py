...
class CalculateSumForm(forms.Form):
    ...
    start_time = forms.TimeField(widget=AdminTimeWidget, label=_('start time'), input_formats=["%H:%M"])
    end_date = forms.DateField(widget=AdminDateWidget, label=_('end date'), input_formats=["%d.%m.%Y"])
    end_time = forms.TimeField(widget=AdminTimeWidget, label=_('end time'), input_formats=["%H:%M"])
...
class CreateNewEventForm(forms.Form):
    ...
    time_of_the_event = forms.TimeField(widget=AdminTimeWidget, label=_('time of the event'),
